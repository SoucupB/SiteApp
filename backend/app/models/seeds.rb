class Seeds < ApplicationRecord
    ...
    def self.generate_products_script(dictionary, collection)
        collection = Colection.new(colection_type: collection[:colectie], description: collection[:descriere], products_ids: [])
        if !collection.save
            return 0
        end
